...
def make_prediction(img, face_detector, anti_spoof):
    ...
    bbox_list = face_detector([img])  # Get the list of bounding boxes

    # Check if bbox_list is empty or not
    if not bbox_list or not isinstance(bbox_list[0], np.ndarray):
        return None

    bbox = bbox_list[0]  # Get the first bounding box
    if bbox.shape[0] == 0:
        return None
...
if __name__ == "__main__":
    ...
    p.add_argument(
        ...
        default=0.5,
        help="real face probability threshold above which the prediction is "
        "considered true",
    )
    args = p.parse_args()
